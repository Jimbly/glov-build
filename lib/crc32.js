...
function update_crc_string(crc, buf, len) {
  for (let n = 0; n < len; n++) {
    crc = crc_table[(crc ^ buf.charCodeAt(n)) & 0xff] ^ (crc >>> 8);
  }
  return crc;
}

// Return the CRC of the bytes buf[0..len-1].
// Note: does not perfectly handle strings with character codes > 255
function crc32(buf, len) {
  len = len || buf.length;
  if (typeof buf === 'string') {
    return (update_crc_string(0xffffffff, buf, len) ^ 0xffffffff) >>> 0;
  } else {
