...
// Escape implicit try/catches from callbacks fired within Promises
function unpromisify(f) {
  return function () {
    // eslint-disable-next-line no-invalid-this, prefer-rest-params
    process.nextTick(f.apply.bind(f, this, arguments));
  };
}
...
function writeFileWithMkdir(gb, disk_path, contents, next) {
  fs.writeFile(disk_path, contents, function (err) {
    ...
    if (err && (err.code === 'ENOENT' || err.code === 'EPERM')) {
      ...
      if (cbs) {
        ...
        gb.debug(`Directory "${dirname}" does not exist, creating...`);
...
function asyncLoader(load_fn) {
  ...
  loader.reset = function (obj, is_loaded, err) {
    assert(!obj[sym_load_cbs]); // load currently in progress
    obj[sym_err] = err;
    obj[sym_loaded] = Boolean(is_loaded);
  };
  return loader;
}
