...
const { asyncLoader, writeFileWithMkdir } = require('./util.js');
...
BuildFile.prototype.getStat = function (next) {
  this.files.getStat(this.bucket, this.path, next);
};
...
function statFile(files, file, next) {
  ...
  fs.stat(disk_path, function (err, stats) {
    if (err) {
      console.error(`Could not stat file ${disk_path}:`, err);
      return next(err);
    }
...
Files.prototype.getStat = function (bucket_name, relative, cb) {
  assert(bucket_name);
  assert(relative);
  assert(relative.indexOf(':') === -1);
  assert.equal(typeof cb, 'function');
...
Files.prototype.put = function (opts, cb) {
  ...
  if (file && file.contents) {
    return cb(new Error(`File "${key}" was already output`));
  }
  if (!file) {
