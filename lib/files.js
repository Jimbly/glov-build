...
function BuildFile(files, bucket_name, relative) {
  ...
  // is_updated set when loaded/stat'd, cleared by build system
...
Files.prototype.fsEvent = function (event, bucket_name, relative, opt_stat) {
  // TODO: what about getStat()s already in-flight?
  if (event === 'unlink') {
    this.registerDelete(bucket_name, relative);
  } else if (event === 'change' || event === 'add') {
    // Just remove from the cache, we'll stat the disk again
    // PERFTODO: use provided opt_stat if available, prevent a second stat
    // PERFTODO: use this.loader/statter.reset() to keep the existing BuildFile objects around.
    this.registerDelete(bucket_name, relative);
  } else {
