...
exports.Files = Files;
exports.isBuildFile = isBuildFile;
exports.BuildFile = BuildFile;
exports.filesGetBucket = filesGetBucket;
...
BuildFile.prototype.serializeForFork = function () {
  return {
    bucket: this.bucket,
...
BuildFile.prototype.deserializeForFork = function (data) {
  assert.equal(this.bucket, data.bucket);
  assert.equal(this.relative, data.relative);
...
function filesGetBucket(files, bucket_name) {
  ...
  files.statter(file, function (err) {
    ...
    readFileWithRetries(files, disk_path, function (err, buf) {
      if (err) {
        ...
        files.gb.silly(`  Read ${disk_path} (${buf.length} bytes)`);
        file.err = null;
        file.contents = buf;
      }
      next(err);
...
Files.prototype.put = function (opts, cb) {
  ...
  let bucket = filesGetBucket(this, opts.bucket);
