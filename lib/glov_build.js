...
const { callbackify, cmpTaskPhase, deleteFileWithRmdir, empty, forwardSlashes, merge, ridx } = require('./util.js');
  ...
  this.exit_handler = () => {
    ...
    for (let key in this.tasks) {
      ...
      if (task.active &&
        ...
        console.warn(`Warning: Task "${task.name}" still active (status=${task.status}) upon process exit`);
        any_active = true;
      }
    }
    if (any_active) {
      console.warn('A job or init callback probably failed to signal completion');
    }
  };
}
...
function BuildTask(gb, opts) {
  ...
  if (this.input) {
    ...
    for (let ii = 0; ii < this.input.length; ++ii) {
      ...
      if (split.length === 1) {
        ...
        bucket = split[0];
        glob = split[1];
        if (this.deps.indexOf(bucket) === -1) {
          this.deps.push(bucket);
        }
      }
  ...
  if (this.target) {
    ...
    this.outdir = path.join(this.taskdir, 'out');
...
BuildTask.prototype.reset = function () {
  ...
  this.err = null;
  this.task_state = createTaskState({ dir: this.taskdir, name: this.name });
  this.jobs = null;
...
BuildJob.prototype.log = function (msg) {
  assert.equal(typeof msg, 'string');
  console.log(chalk.black.bold(`  Task "${this.task.name}", Job "${this.name}": ${msg}`));
};
...
function jobOutputFiles(job, cb) {
  ...
  async.eachOf(job.output_queue, function (file, key, next) {
    ...
    taskOutputFsEvent(job.task, 'add', buildkey);
    job.gb.files.put({
...
BuildJob.prototype.done = function (err) {
  ...
  if (!this.waiting) {
    ...
    jobOutputFiles(this, (num_files, num_pruned) => {
      ...
      // should probably just print out the previous errors for SINGLE jobs,
      // and ALL jobs handle it themselves, or we add per-file error states,
      // that just keeps particular files dirty.
      if (!any_errors || true) {
        for (let ii = 0; ii < this.files_updated.length; ++ii) {
          this.files_updated[ii].is_updated = false;
        }
...
function taskGatherInputsFirstRun(gb, task) {
  ...
  async.mapValuesSeries(task.globs_by_bucket, function (globs, bucket, next) {
    if (bucket === 'source') {
      // Raw from filesystem
      fg(globs, {
      ...
      // TODO: Need to call gb.files.get on each to actually load the data?
...
function taskPrepJobsFirstRun(gb, task, input_files) {
  ...
  async.eachOfLimit(jobs, 1, function (job, job_name, next) {
    job.isUpToDate(function (err) {
      if (err) {
        // needs updating
        job.dirty = true;
      } else {
        console.debug(chalk.black.bold(`  Task "${task.name}", Job "${job.name}": up to date`));
      }
      next();
    });
  }, done);
...
function taskGatherInputsDynamic(gb, task) {
  ...
  for (let key in fs_events) {
    ...
    let the_file = gb.files.getPre(bucket, relative);
    if (event === 'unlink') {
      // Deleted file:
      if (task.type === gb.SINGLE) {
        ...
        let job = jobs[key];
        if (job) {
          console.debug(`  Task "${task.name}", Job "${job.name}": input deleted, pruning...`);
      ...
      for (let job_name in jobs) {
        ...
        let job_state = job.last_job_state;
        if (job_state.deps[key]) {
          console.debug(chalk.black.bold(`  Task "${task.name}", Job "${job.name}": dep deleted: ${key}`));
      ...
      if (globs) {
        if (micromatch.isMatch(relative, globs)) {
          if (task.type === gb.SINGLE) {
            ...
            if (!job) {
              ...
              job.dirty = true;
              console.debug(chalk.black.bold(`  Task "${task.name}", Job "${job.name}": new job from new input`));
              logged = true;
            } else {
              ...
              jobFileListUpdate(job, 'files_all', the_file, true);
              jobFileListAdd(job, 'files_updated', the_file);
              console.debug(chalk.black.bold(`  Task "${task.name}", Job "${job.name}": base file modified`));
              logged = true;
            }
            ...
            jobFileListAdd(job, 'files_updated', the_file);
            if (is_new) {
              console.debug(chalk.black.bold(`  Task "${task.name}", Job "${job.name}": new input: ${key}`));
              logged = true;
            } else {
              console.debug(chalk.black.bold(`  Task "${task.name}", Job "${job.name}": input modified: ${key}`));
              logged = true;
            }
          }
        }
      ...
      for (let job_name in jobs) {
        ...
        if (job_state && job_state.deps[key]) {
          if (job.job_has_run) {
            ...
            jobFileListAdd(job, 'files_updated', the_file);
            if (!logged) {
              console.debug(chalk.black.bold(`  Task "${task.name}", Job "${job.name}": dep modified: ${key}`));
            }
          } else {
            if (!logged) {
              console.debug(chalk.black.bold(`  Task "${task.name}", Job "${job.name}":` +
...
function taskInit(task, next) {
  ...
  task.post_init = [next];
  let start = Date.now();
  console.log(`  Task "${task.name}": initializing...`);
  task.init(function () {
    console.log(`  Task "${task.name}": initialized in ${Date.now() - start}ms`);
    task.did_init = true;
...
function taskPrepDeps(gb, task) {
  ...
  async.eachLimit(files, ASYNC_LIMIT, function (file, next) {
    file.get(function (ignored) {
      next();
    });
  }, function (err) {
    assert(!err, err);
    executeTask(gb, task, dirty_jobs);
  });
}
...
function executeTask(gb, task, dirty_jobs) {
  ...
  let error_count = 0;
  async.eachLimit(dirty_jobs, ASYNC_LIMIT, function (job, next) {
    job.on_done = function (err) {
    ...
    if (!err && error_count) {
      err = `${error_count} error${error_count === 1 ? '' : 's'}`;
    }
    if (err) {
      return taskSetErr(task, err);
    }
...
function executeTaskOld(gb, task, inputs) {
  assert.equal(task.status, STATUS_PREPARING_INPUTS);
  let is_dynamic = gb.watcher && task.jobs;
  let jobs = task.jobs || Object.create(null);
  let expected = Object.create(null);
  if (task.type === SINGLE) {
    // make a job for each input
    for (let ii = 0; ii < inputs.length; ++ii) {
      let the_file = inputs[ii];
      let key = the_file.key;
      expected[key] = true;
      let job = jobs[key];
      if (!job) {
        jobs[key] = new BuildJob(gb, task, key, [the_file]);
      } else {
        // apply files - TODO: this should happen much earlier, in file change?
        assert.equal(job.files_base[0], the_file);
        if (the_file.isUpdated()) {
          if (job.files_updated.indexOf(the_file) === -1) {
            job.files_updated.push(the_file);
            job.need_sort = true;
          }
        }
      }
    ...
    let job = jobs.all;
    if (!job) {
      jobs.all = new BuildJob(gb, task, 'all', inputs);
    } else {
      // apply files - TODO: this should happen much earlier, in file change?
      let expected_keys = Object.create(null);

      for (let ii = 0; ii < inputs.length; ++ii) {
        let the_file = inputs[ii];
        expected_keys[the_file.key] = true;
        if (job.files_all.indexOf(the_file) === -1) {
          job.files_all.push(the_file);
          job.files_updated.push(the_file);
          for (let jj = 0; jj < job.files_deleted.length; ++jj) {
            if (job.files_deleted[jj].key === the_file.key) {
              ridx(job.files_deleted, jj);
              break;
            }
          }
          job.need_sort = true;
        } else if (the_file.isUpdated()) {
          if (job.files_updated.indexOf(the_file) === -1) {
            job.files_updated.push(the_file);
            job.need_sort = true;
          }
      ...
      for (let ii = job.files_all.length - 1; ii >= 0; --ii) {
        let the_file = job.files_all[ii];
        if (!expected_keys[the_file.key]) {
          ridx(job.files_all, ii);
          let idx = job.files_updated.indexOf(the_file);
          if (idx !== -1) {
            ridx(job.files_updated, idx);
          }
          job.files_deleted.push(the_file);
          job.need_sort = true;
        }
      }
    }
  ...
  if (is_dynamic) {
    // potentially doing dynamic processing, keep the jobs around, merge with
    //   previous jobs
    for (let key in jobs) {
      if (!expected[key]) {
        delete jobs[key];
  ...
  if (gb.watcher) {
    task.jobs = jobs; // Persist for dynamic updating later
  }
  taskSetStatus(task, STATUS_RUNNING, `${Object.keys(jobs).length} jobs`);
  task.did_init = !(task.init || task.finish);
  task.post_init = null;
  // TODO: add this to a global job queue for rate limiting?
  // TODO: async.eachLimit doesn't gracefully handle an error - other jobs keep
...
function executeJob(gb, task, job) {
  ...
  if (!job.job_has_run) {
    ...
    for (let ii = 0; ii < job.files_updated.length; ++ii) {
      ...
      assert(job.files_updated[ii].contents);
    }
  }
  ...
  taskInit(task, function () {
    for (let ii = 0; ii < job.files_updated.length; ++ii) {
      ...
      assert(file.err || file.contents); // should already be loaded
    }
    job.dirty = false;
    gb.stats.jobs++;
    let is_done = false;
    task.func(job, function (err) {
      assert(!is_done, `done() called twice for Task "${task.name}", job "${job.name}"`);
      is_done = true;
...
function fsEvent(gb, watch_generation, event, relative, opt_stat) {
  ...
  // Let files know
  if (!gb.files.fsEvent(event, 'source', relative, opt_stat)) {
    console.log(chalk.black.bold(`Detected spurious file change: ${relative} (${event})`));
  ...
  // TODO: just queue these fs events to each relevant task, and it can apply
  //   them in taskGatherInputs.

  // Find all tasks that depend on this file
  let invalidated_tasks = {};
  let short_list = {};
  ...
  let impacted = Object.keys(short_list).join(',');
  console.log(chalk.black.bold(`Detected file change: ${relative} (${event});` +
    ` impacted tasks: ${impacted || 'none'}`));

  if (!impacted) {
    return;
  }

  // Try to reset them and their active dependors to pending
  //   If they were anything other than stopped or pending, we need to stop their task first
  // STATUS_PREPARING_INPUTS - stop and reset when done
  // STATUS_RUNNING - stop and reset when done
  for (let task_name in invalidated_tasks) {
    let task = tasks[task_name];
    if (task.status === STATUS_PENDING) {
      // no change
    } else if (STATUS_IS_STOPPED[task.status]) {
      // reset to pending
      taskSetStatus(task, STATUS_PENDING, short_list[task_name] ? 'source file changed' : 'dependency pending');
    } else {
      assert(false, `TODO: implement task status resetting for ${task.status} tasks`);
    }
  }
...
GlovBuild.prototype.go = function (opts) {
  ...
  if (watch) {
    ...
    this.watcher.on('add', fsEvent.bind(null, this, gen, 'add'));
    this.watcher.on('change', fsEvent.bind(null, this, gen, 'change'));
    this.watcher.on('unlink', fsEvent.bind(null, this, gen, 'unlink'));
  }
