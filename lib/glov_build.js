...
function taskInitToFork(task, next) {
...
function taskInit(task, next) {
  if (!fork && task.async === ASYNC_FORK && task.gb.config.allow_fork) {
    return taskInitToFork(task, next);
  }
  if (task.did_init) {
    return next();
  }
...
function taskGetChildProc(task) {
  ...
  if (!worker_task.child) {
    worker_task.child = taskStartFork(task.gb);
  }
  return worker_task.child;
}

function executeTaskSendToFork(task) {
  assert.equal(task.status, STATUS_RUNNING);
  let { dirty_jobs } = task;
  let child = taskGetChildProc(task);
  let file_list = [];
...
function forkedTaskInit(gb, obj) {
...
function executeTask(gb, task, dirty_jobs) {
  ...
  if (task.async === ASYNC_FORK && gb.config.allow_fork) {
    return executeTaskSendToFork(task);
  }
