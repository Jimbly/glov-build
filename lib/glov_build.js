...
function jobPrintWarning(job, msg) {
  console.warn(chalk.yellow.bold(`  Task "${job.task.name}", Job "${job.name}": warning:`), msg);
...
function jobPrintError(job, err) {
  ...
  if (err instanceof Error && err.stack) {
    ...
    ret = stack[0];
    console.error(chalk.red.bold(`  Task "${job.task.name}", Job "${job.name}": error:`),
      stack.join('\n  '));
  } else {
    err = String(err);
    ret = err;
...
function taskInit(task, next) {
  ...
  task.init(function () {
    console.log(`  Task "${task.name}": initialized in ${time(Date.now() - start)}`);
    task.did_init = true;
