...
const async = require('async'); // TODO: don't use this
const { asyncEach, asyncEachLimit, asyncEachSeries, asyncLimiter, asyncSeries } = require('glovjs-async');
...
function taskSetStatus(task, status, message) {
  ...
  let log_msg = `Task "${task.name}": ${task.status}->` +
    ...
    asyncEachLimit(Object.keys(coll), ASYNC_LIMIT, (key, next) => {
      ...
      gb.files.getStat(...parseBucket(key), `${this.task.name}:${this.name}`, (err, file) => {
        ...
        if (file.timestamp !== timestamp) {
          ldebug(`  Task "${this.task.name}", Job "${this.name}": "${key}" ` +
...
function taskInit(task, next) {
  ...
  task.init(function () {
    ...
    if (!task.ever_did_init || dt > 750) {
      linfo(`  Task "${task.name}": initialized in ${time(dt)}`);
    }
...
function executeTask(gb, task, dirty_jobs) {
  ...
  asyncEachLimit(dirty_jobs, ASYNC_LIMIT, function (job, next) {
    ...
    taskSetStatus(task, STATUS_DONE, plural(task.count_outputs, 'output'));
    scheduleTick(gb);
  });
}
...
function tick(gb) {
  ...
  if (!gb.was_all_done && all_tasks_done) {
    if (gb.watcher) {
      ...
      gb.info(chalk[any_errored ? 'red' : 'green'](`All tasks complete in ${time(dt)}, ` +
...
function doClean(gb) {
  ...
  asyncSeries([
    function cleanJobs(next) {
      asyncEachSeries(gb.tasks, function (task, next) {
...
function fsEvent(gb, watch_generation, event, relative, opt_stat) {
  ...
  linfo(`Detected file change: ${relative} (${event});` +
