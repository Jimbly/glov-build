...
const { filesCreate, isBuildFile } = require('./files.js');
const { max } = Math;
  ...
  this.exit_handler = () => {
    ...
    for (let key in this.tasks) {
      ...
      if (task.active &&
        ...
        lnotice(`Warning: Task "${task.name}" still active (status=${task.status}) upon process exit`);
        any_active = true;
      }
    }
    if (any_active) {
  ...
  if (this.input) {
    ...
    for (let ii = 0; ii < this.input.length; ++ii) {
      ...
      if (split.length === 1) {
        ...
        bucket = split[0];
        glob = split[1];
        if (bucket !== 'source' && this.deps.indexOf(bucket) === -1) {
...
function taskSetStatus(task, status, message) {
  ...
  let log_msg = `Task "${task.name}": ${task.status}->${status}${message ? ` (${message})` : ''}`;
...
function taskSetErr(task, err) {
  task.err = err;
  lerror(`Task "${task.name}" error:`, err);
...
BuildJob.prototype.debug = function (msg, unexpected) {
  ...
  ldebug(`  Task "${this.task.name}", Job "${this.name}": ${msg}`);
};
BuildJob.prototype.log = function (msg, unexpected) {
  assert(!unexpected);
  assert.equal(typeof msg, 'string');
  linfo(`  Task "${this.task.name}", Job "${this.name}": ${msg}`);
};
function jobPrintWarning(job, msg) {
  lwarn(`  Task "${job.task.name}", Job "${job.name}": warning:`, msg);
...
function jobPrintError(job, err) {
  ...
  if (err instanceof Error && err.stack) {
    ...
    ret = stack[0];
    lerror(`  Task "${job.task.name}", Job "${job.name}": error:`,
      stack.join('\n  '));
  } else {
    err = String(err);
    ret = err;
  ...
  if (!this.waiting) {
    ...
    jobOutputFiles(this, (num_files, num_pruned) => {
      ...
      this.task.task_state.setJobState(this.name, this.job_state, (err) => {
        ...
        if (this.errors.length) {
          linfo(`  Task "${this.task.name}", Job "${this.name}": failed`);
        } else {
          linfo(`  Task "${this.task.name}", Job "${this.name}": complete (` +
    ...
    // } else {
    //   ldebug(`  Task "${this.task.name}", Job "${this.name}": up to date`);
...
BuildJob.prototype.isUpToDate = function (cb) {
  ...
  if (task.version !== (version || 0)) {
    linfo(`  Task "${this.task.name}", Job "${this.name}": ` +
  ...
  for (let ii = 0; ii < files_updated.length; ++ii) {
    ...
    if (!deps[file.key]) {
      linfo(`  Task "${this.task.name}", Job "${this.name}": "${file.key}"` +
  ...
  async.each({ deps, outputs }, (coll, next) => {
    async.eachOfLimit(coll, ASYNC_LIMIT, (timestamp, key, next) => {
      gb.files.getStat(...parseBucket(key), `${this.task.name}:${this.name}`, (err, file) => {
        if (err) {
          linfo(`  Task "${this.task.name}", Job "${this.name}": "${key}" missing or errored, updating...`);
          return next(err);
        }
        if (file.timestamp !== timestamp) {
          linfo(`  Task "${this.task.name}", Job "${this.name}": "${key}" changed, updating...`);
...
function taskPrepJobsFirstRun(gb, task, input_files) {
  ...
  for (let job_name in job_states) {
    if (!expected[job_name]) {
      linfo(`  Task "${task.name}", Job "${job_name}": not in new input, pruning...`);
  ...
  async.eachOfLimit(jobs, 1, function (job, job_name, next) {
    job.isUpToDate(function (err) {
      if (err) {
        ...
        lsilly(`  Task "${task.name}", Job "${job.name}": up to date`);
      }
      next();
    });
  }, done);
  ...
  for (let key in fs_events) {
    ...
    if (event === 'unlink') {
      ...
      if (task.type === gb.SINGLE) {
        ...
        let job = jobs[key];
        if (job) {
          linfo(`  Task "${task.name}", Job "${job.name}": input deleted, pruning...`);
      ...
      for (let job_name in jobs) {
        ...
        if (job_state.deps[key]) {
          linfo(chalk.black.bold(`  Task "${task.name}", Job "${job.name}": dep deleted: ${key}`));
        ...
        if (micromatch(relative, globs).length) {
          if (task.type === gb.SINGLE) {
            ...
            if (!job) {
              ...
              job.dirty = true;
              ldebug(`  Task "${task.name}", Job "${job.name}": new job from new input`);
              logged = true;
            } else {
              ...
              jobFileListUpdate(job, 'files_all', the_file, true);
              jobFileListAdd(job, 'files_updated', the_file);
              ldebug(`  Task "${task.name}", Job "${job.name}": base file modified`);
              logged = true;
            }
            ...
            jobFileListAdd(job, 'files_updated', the_file);
            if (is_new) {
              ldebug(`  Task "${task.name}", Job "${job.name}": new input: ${key}`);
              logged = true;
            } else {
              ldebug(`  Task "${task.name}", Job "${job.name}": input modified: ${key}`);
              logged = true;
            }
          }
        }
      ...
      for (let job_name in jobs) {
        ...
        if (job_state && job_state.deps[key]) {
          if (job.job_has_run) {
            ...
            jobFileListAdd(job, 'files_updated', the_file);
            if (!logged) {
              ldebug(`  Task "${task.name}", Job "${job.name}": dep modified: ${key}`);
            }
          } else {
            if (!logged) {
              ldebug(`  Task "${task.name}", Job "${job.name}":` +
...
function taskInit(task, next) {
  ...
  task.post_init = [next];
  let start = Date.now();
  linfo(`  Task "${task.name}": initializing...`);
  task.init(function () {
    linfo(`  Task "${task.name}": initialized in ${time(Date.now() - start)}`);
    task.did_init = true;
...
function fsEvent(gb, watch_generation, event, relative, opt_stat) {
  ...
  ldebug(`Detected file change: ${relative} (${event});` +
