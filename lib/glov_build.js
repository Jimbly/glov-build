...
function doTaskComplete(gb, task, obj) {
  assert.equal(obj.task_name, task.name);
  executeTaskComplete(gb, task, task.post_waiting_err || obj.err);
}

let last_worker_id = 0;
function taskStartFork(gb) {
  let child = child_process.fork(process.argv[1], ['--fork'].concat(gb.argv_raw), {
      ...
      switch (obj.cmd) {
        ...
        case 'task_complete': {
          ...
          if (task.waiting_on_fork) {
            task.post_waiting = doTaskComplete.bind(null, gb, task, obj);
          } else {
            doTaskComplete(gb, task, obj);
          }
...
function taskGetChildProc(gb, task) {
  let worker_task = gb.tasks[task.async_task];
  if (!worker_task.child) {
    worker_task.child = taskStartFork(gb);
  }
...
function executeTaskSendToFork(gb, task) {
  assert.equal(task.status, STATUS_RUNNING);
  let { dirty_jobs } = task;
  let child = taskGetChildProc(gb, task);
  let file_list = [];
