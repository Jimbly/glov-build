  ...
  for (let key in fs_events) {
    ...
    if (event === 'unlink') {
      ...
      for (let job_name in jobs) {
        ...
        if (job_state && (
          job_state.deps[key] ||
          jobFileListFind(job, 'files_all', key) !== -1
        )) {
...
function jobExecute(gb, task, job, next) {
  ...
  taskInit(task, function () {
    ...
    for (let ii = 0; ii < job.files_base.length; ++ii) {
      ...
      if (file.err) {
        job.error(file, `${file.bucket}:${file.relative}: ${file.err}`);
        base_error = true;
      }
    }
    if (base_error) {
...
function doClean(gb, dry_run) {
  ...
  //   in migration?
  let orphans = gb.argv.orphans || gb.argv.prune;
  lnotice(`Performing ${orphans ? 'orphans prune' : 'clean'}${dry_run ? ' (dry run)' : ''}...`);
  let handled_files = Object.create(null);
  asyncSeries([
    function cleanJobs(next) {
      asyncEachSeries(Object.values(gb.tasks), function (task, next) {
        task.task_state.loadAll(function () {
          if (orphans || dry_run) {
            // for dry_run, assume this file will be pruned after deleting all contents
            // for orphans, this file should be left alone
            // either way, handled!
            let disk_path = forwardSlashes(path.join(task.task_state.dir, 'state.json'));
            handled_files[disk_path] = true;
          }
          let job_states = task.task_state.getAllJobStates();
          asyncEachSeries(Object.keys(job_states), function (job_name, next) {
            ...
            if (empty(outputs)) {
              ...
              if (orphans) {
                // don't clean job state
                return next();
              } else if (dry_run) {
                linfo(`  Would clear job state for job with no outputs: ${job_name}`);
                return next();
              }
            ...
            asyncEachSeries(Object.keys(outputs), function (key, next) {
              if (orphans || dry_run) {
                // just track somewhere
                let [ bucket, relative ] = parseBucket(key);
                let disk_path = forwardSlashes(path.join(gb.files.bucket_dirs[bucket], relative));
                handled_files[disk_path] = true;
                if (!orphans) {
                  linfo(`  Would delete ${disk_path}`);
                }
                return next();
              }
    ...
    function cleanTargets(next) {
      asyncEachSeries(Object.keys(gb.config.targets), function (target_name, next) {
        ...
        readdirRecursive(target_dir, function (err, files) {
          ...
          files = (files || []).map(forwardSlashes);
          files = files.filter(function (filename) {
            return !handled_files[filename];
          });
          if (!files.length) {
            return next();
          }
          if (gb.argv.force || orphans) {
            asyncEachSeries(files, function (filename, next) {
              if (!logged) {
                logged = true;
                lnotice(`${orphans ? 'Pruning orphans' : 'Cleaning remaining files'} in target dir${dry_run ? ' (dry run)' : ''}...`);
              }
              if (dry_run) {
                linfo(`  Would delete ${filename}`);
      ...
      readdirRecursive(statedir, function (err, files) {
        ...
        if (gb.argv.force || orphans) {
          asyncEachSeries(files, function (filename, next) {
            if (!logged) {
              logged = true;
              lnotice(`${orphans ? 'Orphans prune' : 'Clean'}${dry_run ? ' (dry run)' : ''} of leftover state...`);
            }
            if (dry_run) {
              linfo(`  Would delete ${filename}`);
              process.nextTick(next);
