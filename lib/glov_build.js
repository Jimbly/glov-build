...
function BuildTask(gb, opts) {
  ...
  this.user_data = opts.on_end || opts.on_start ? {} : null;
  ...
  if (!this.version && opts.func) {
    ...
    this.version = `CRC#${crc32(opts.func.toString()).toString(16)}`;
  }
...
BuildJob.prototype.getTaskUserData = function () {
  assert(this.task.user_data, 'getTaskUserData() only valid if the task has an on_start/on_end hook');
