...
function BuildTask(gb, opts) {
  ...
  this.task_state = createTaskState({ dir: taskdir });
  if (this.target) {
...
function taskSetStatus(task, status, message) {
  ...
  console.log(`Task "${task.name}": ${task.status}->${status}${message ? ` (${message})` : ''}`);
  task.status = status;
}
...
BuildJob.prototype.warn = function (msg) {
  ...
  console.warn(`  Task "${this.task.name}", Job "${this.name}": warning: ${msg}`);
};
BuildJob.prototype.error = function (err) {
  ...
  console.error(`  Task "${this.task.name}", Job "${this.name}": error:`, err);
};
...
BuildJob.prototype.done = function (err) {
  ...
  if (!this.waiting) {
    if (this.job_state !== this.last_job_state) {
      ...
      jobOutputFiles(this, (num_files) => {
        if (this.errors.length) {
          this.job_state.errors = this.errors.map((e) => `${e}`);
        } else {
      ...
      console.debug(`  Task "${this.task.name}", Job "${this.name}": up to date`);
...
BuildJob.prototype.isUpToDate = function (cb) {
  ...
  let { deps, outputs } = last_job_state;
  ...
  async.each({ deps, outputs }, (coll, next) => {
    async.eachOfLimit(coll, ASYNC_LIMIT, (timestamp, key, next) => {
      gb.files.getStat(...parseBucket(key), (err, file) => {
        if (err) {
    ...
    if (bucket === 'source') {
      ...
      fg(globs, {
        ...
        async.mapLimit(entries, ASYNC_LIMIT, function (file, next) {
          gb.files.getStat(bucket, file.path, function (err, buildfile) {
            if (err) {
              return next(err);
            }
...
function taskGatherDeps(gb, task, input_files) {
  ...
  // This is not strictly needed, but helps with managing stages/phases so that all
  // async dep querying will be done before a single user-level function is ran?
  ...
  function statFile(filename) {
    ...
    ++waiting;
    gb.files.getStat(...parseBucket(filename), done);
  }
