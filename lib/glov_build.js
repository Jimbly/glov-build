...
BuildJob.prototype.depReset = function () {
  ...
  for (let key in this.job_state.deps) {
    if (!expected_deps[key]) {
      ...
      // TODO: also prune from files_updated, files_all, and files_deleted!
...
function taskGatherInputs(gb, task) {
  ...
  if (task.task_has_ran) {
    assert(!'todo');
  } else {
    task.task_has_ran = true;
...
function executeJob(gb, task, job) {
  assert(job.dirty);
  if (!job.job_has_ran) {
    // Fresh state should always be: no deleted, and files_all == files_base == files_updated
    assert.equal(job.files_deleted.length, 0);
...
function fsEvent(gb, watch_generation, event, relative, opt_stat) {
  ...
  function flag(task) {
    ...
    for (let ii = 0; ii < task.dependors_active.length; ++ii) {
      flag(task.dependors_active[ii]);
    }
  }
  let { tasks } = gb;
  for (let task_name in tasks) {
    let task = tasks[task_name];
    if (!task.active || !task.task_has_ran) {
      continue;
    }
