...
function log(level, msg, ...args) {
  ...
  if (fork) {
    process.send({
      cmd: 'log',
      level, msg: `[worker:${worker_id || process.pid}] ${msg}`, args,
    });
    return;
  }
...
BuildJob.prototype.serializeForFork = function (file_list) {
  ...
  return {
    name: this.name,
    files_all: this.files_all.map(serializeFile),
    files_base: this.files_base.map(lookupFile),
    files_updated: this.files_updated.map(lookupFile),
  };
};
...
BuildJob.prototype.out = function (file) {
  ...
  if (this.output_queue[key]) {
    this.error(file, new Error(`Job is outputting the same file ("${key}") twice`));
  } else {
...
function taskStartFork(gb, task) {
  let child = task.child = child_process.fork(process.argv[1], ['--fork'].concat(gb.argv_raw), {
  ...
  let waiting = 0;
  let post_waiting;
  function doTaskComplete(obj) {
    if (!gb.watcher) {
      taskEndChild(task);
    }
    assert.equal(obj.task_name, task.name);
    executeTaskComplete(gb, task, task_err || obj.err);
  }
  child.on('message', (obj) => {
    if (obj && obj.cmd) {
      ...
      switch (obj.cmd) {
        case 'log':
          return log(obj.level, obj.msg, ...obj.args);
        case 'job_dep_reset': {
          assert.equal(task.status, STATUS_RUNNING);
          let job = task.jobs[obj.name];
          assert(job);
          job.depReset();
        } return;
        case 'job_dep_add': {
          ...
          depAddInternal(job, obj.bucket, obj.relative, (file) => {
            child.send({
              cmd: 'response',
              resp_id: obj.resp_id,
        ...
        case 'task_complete': {
          ...
          if (waiting) {
            post_waiting = doTaskComplete.bind(null, obj);
          } else {
            doTaskComplete(obj);
          }
        } return;
...
function executeTaskSendToFork(gb, task) {
  assert.equal(task.status, STATUS_RUNNING);
  let { child, dirty_jobs } = task;
  if (!child) {
    taskStartFork(gb, task);
    child = task.child;
  }
  let file_list = [];
  let ser_dirty_jobs = dirty_jobs.map((job) => job.serializeForFork(file_list));
  task.async_file_refs = file_list;
...
function forkedExecuteTask(gb, task_data) {
...
function executeTaskComplete(gb, task, err) {
...
function executeTask(gb, task, dirty_jobs) {
  ...
  asyncEachLimit(dirty_jobs, ASYNC_LIMIT, function (job, next) {
    ...
    executeTaskComplete(gb, task, err);
  });
}
...
function idleCheck(gb) {
  ...
  for (let task_name in tasks) {
    let task = tasks[task_name];
    if (task.init && !task.did_init && task.active) {
      candidate = task;
      break;
    }
  }
...
function abortingUpdateForkedTasks(gb) {
  ...
  for (let task_name in tasks) {
    let task = tasks[task_name];
    if (task.async === ASYNC_FORK && task.child) {
